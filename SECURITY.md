...
Within `0.2.x`, take `0.2.1` or later: `0.2.0` could print the key from options that are an
improper list, from an entry `{:private_key, key, extra}`, or from a key function that raised
(the 0.2.1 CHANGELOG entry).
